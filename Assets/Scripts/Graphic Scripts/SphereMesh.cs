...
public class SphereMesh
{
    ...
    public static MeshData GenerateMeshData(int resolution, float radius = 1)
    {
        ...
        Vector3[] normals = new Vector3[vertices.items.Length];
        for (int i = 0; i < vertices.items.Length; i++)
            normals[i] = vertices.items[i];

        for (int i = 0; i < vertices.items.Length; i++)
            vertices.items[i] *= radius;

        MeshData meshData = new MeshData(vertices.items, triangles.items, normals, name: "Ico Sphere");
        return meshData;
    ...
    public static int NumVerticesFromResolution(int resolution)
    {
        int numDivisions = Mathf.Max(0, resolution);
        int numVertsPerFace = ((numDivisions + 3) * (numDivisions + 3) - (numDivisions + 3)) / 2;
        int numVerts = numVertsPerFace * 8 - (numDivisions + 2) * 12 + 6;
